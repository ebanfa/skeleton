...
func (_m *DatabaseFactory) CreateDatabase(name string, path string) (types.Database, error) {
	...
	var r0 types.Database
	var r1 error
	if rf, ok := ret.Get(0).(func(string, string) (types.Database, error)); ok {
		return rf(name, path)
	}
	if rf, ok := ret.Get(0).(func(string, string) types.Database); ok {
		r0 = rf(name, path)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(types.Database)
		}
	}
